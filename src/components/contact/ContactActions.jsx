...
const ContactActions = ({ isArabic, supportHref = '/contact-us', className = '' }) => (
  <section className={`contact-actions ${className}`.trim()} aria-label={isArabic ? 'خيارات التواصل' : 'Contact options'}>
    <a href={supportHref} className="contact-actions__cta contact-actions__cta--support" aria-label={isArabic ? 'التواصل مع الدعم' : 'Contact support'}>
    ...
    <a href={ENGINEERS_WHATSAPP_URL} target="_blank" rel="noreferrer" className="contact-actions__cta contact-actions__cta--development" aria-label={isArabic ? 'التواصل مع فريق التطوير' : 'Contact the development team'}>
