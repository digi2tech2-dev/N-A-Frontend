...
export default function InchillManagementModal({ provider, isOpen, onClose }) {
  ...
  const status = useMemo(() => String(connection?.connectionStatus || 'UNKNOWN').toUpperCase(), [connection?.connectionStatus]);
  const hasConnection = connection?.hasConnection === true;
  const connected = status === 'CONNECTED' && hasConnection;
