...
const registerServiceWorker = () => {
  ...
  // The production service worker must never cache Vite's dev/HMR modules.
  // If an older registration exists, remove it so localhost always loads the
  // current React runtime and source modules from the dev server.
  if (import.meta.env.DEV) {
    ...
    if ('caches' in window) {
      void caches.keys().then((keys) => Promise.all(
        keys
          .filter((key) => key.startsWith('nahub-web-'))
